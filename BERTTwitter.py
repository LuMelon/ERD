...
class SentimentModel(nn.Module):
    ...
    def forward(self, word_ids):
        ...
        cls_feature =torch.cat([item[0][0][0] for item in outs], axis=0).reshape([-1, 768]).cuda()
...
def Train(bert, train_reader, valid_reader, test_reader, hidden_dim, logger, logdir):
    ...
    senti_model = SentimentModel(bert, hidden_dim, 3, 0.8).cuda()
    optim = torch.optim.Adam([
                                {'params': senti_model.bert.parameters(), 'lr':1e-3},
